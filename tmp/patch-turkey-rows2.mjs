...
function injectRowByHeaderContains(xml, headerContains, placeholders){
  // find header cell containing headerContains
  const idx = xml.indexOf(headerContains);
  if (idx < 0) return xml;
  const tOpen = xml.lastIndexOf('<w:t', idx);
  const tClose = xml.indexOf('</w:t>', idx);
  if (tOpen < 0 || tClose < 0) return xml;
  const trEnd = xml.indexOf('</w:tr>', tClose);
...
async function main(){
  ...
  if(!xml.includes('{{workDateFrom}}')){
    xml = injectRowByHeaderContains(xml, 'Employer', ['workDateFrom','workDateTo','workEmployerDetails','workPositionDetails','workReasons','workEndOfContracts']);
  }
  if(!xml.includes('{{languageNames}}')){
    xml = injectRowByHeaderContains(xml, 'Language', ['languageNames','languageSpeaking','languageReading','languageWriting','languageExtras']);
  }

  zip.file('word/document.xml', xml);
  const out = await zip.generateAsync({ type: 'nodebuffer' });
  await fs.writeFile(templatePath, out);
  console.log('Row placeholders injected by contains-header.');
}
