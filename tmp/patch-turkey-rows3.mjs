...
async function main(){
  ...
  if(!xml.includes('{{workDateFrom}}')){
    // anchor on 'From' header instead
    xml = injectRowByHeaderContains(xml, 'From', ['workDateFrom','workDateTo','workEmployerDetails','workPositionDetails','workReasons','workEndOfContracts']);
  }
  ...
  console.log('Row placeholders injected by From/Language anchors.');
}
